...
public class Server {
				...
				if (!clientRequest.isEmpty()) {
					...
					if (indexOfSearch != -1) {
						...
						if (CHALLENGE(rand, indexOfSearch, received.getAddress(), received.getPort())) {
							...
							AUTH_SUCCESS(randNum(), received.getAddress(), received.getPort(), listClient.get(indexOfSearch).getKey(), indexOfSearch);
						}
						else {
	...
	public static void AUTH_SUCCESS(int rand_cookie, InetAddress address, int port, int secretKey, int index) throws Exception {
		...
		// Update subscriber's CK_A value
